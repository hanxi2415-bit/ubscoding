...
def get_next_node(
    ...
    """Return the next node on the cheapest valid directed route.

    The route never revisits a node. Its cost includes every directed edge
    weight and the toll charged when entering the next node. If hops_left is
    provided, the destination must be reachable within that many edges.
    """
