...
# ============================================================
# VALUE SCORING (Phase 3)
# ============================================================

def collect_decay_chain(from_user, max_depth=8):
    """
    Walk backward from `from_user` along the dominant active incoming
    edge at each step (the largest amount, when a node has more than
    one incoming edge), returning the ordered list of amounts from the
    immediate predecessor edge back toward the origin of the flow.

    This reconstructs the value trajectory that a new outgoing
    transaction from `from_user` is about to join, so it can be checked
    for continuity (Phase 3 Examples 1-2) or a reversal against it
    (Example 3). Convergence (Example 4) is handled naturally: the walk
    only ever follows edges feeding INTO from_user, so a sibling branch
    that later merges downstream never enters this chain.
    """
    chain = []
    current = from_user
    visited = {from_user}
    for _ in range(max_depth):
        incoming = incoming_edges(current)
        if not incoming:
            break
        candidates = [t for t in incoming if t.get("amount") is not None]
        if not candidates:
            break
        best = max(candidates, key=lambda t: t["amount"])
        chain.append(best["amount"])
        nxt = best["fromUserId"]
        if nxt in visited:
            break
        visited.add(nxt)
...
def value_score(tx, from_user, to_user):
    """
    Compares this transaction's amount against the value trajectory
    already established on the active flow feeding into from_user.

    - No upstream amount evidence -> neutral; nothing to compare against.
    - Amount continues the decline (<= the immediate predecessor amount)
      -> the characteristic layering pattern (Examples 1 & 2). This is
      the expected shape of the flow, not a deviation from it, so it
      adds no risk on its own.
    - Amount reverses the trajectory (> the immediate predecessor amount)
      -> a direct contradiction of the expected degradation (Example 3).
      Scored by how large the reversal is and how long a decay run it
      broke -- reversing a well-established multi-hop decline is worse
      than reversing a single step.
    """
    amount = tx.get("amount")
    if amount is None:
        return 0.0

    chain = collect_decay_chain(from_user)
    if not chain:
        return 0.0
    ...
    if amount <= reference:
        return 0.0  # consistent decay -- expected pattern, not an anomaly

    overshoot = (amount - reference) / reference
    chain_run = len(chain)
    base = 0.25 + 0.20 * saturate(overshoot, k=0.02)
    base += 0.08 * saturate(chain_run - 1, k=2.0)
...
def calculate_risk(tx):
    ...
    value = value_score(tx, from_user, to_user)

    score = max(0.0, min(structural + identity + value, 1.0))
